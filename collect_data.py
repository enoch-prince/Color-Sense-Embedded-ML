...
def replace_last_occurrence(main_string, old_substring, new_substring):
    ...
    if index == -1:
        return main_string  # Substring not found
    else:
...
try:
    while True:
        ...
        if ser.in_waiting > 0:
            ...
            if (reading[:3].isalpha()):
                rx_buf += reading.replace(b'\r\n', b',color_label\r\n')
            elif (reading[:3].isdigit() or isinstance(reading[0], int)):
                rx_buf += reading.replace(b'\r\n', (","+label+"\r\n").encode())
