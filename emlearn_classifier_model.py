...
def load_data(input_dir: str) -> pandas.DataFrame:
    ...
    if "combined_color_data.csv" in os.listdir("data"):
        ...
        combined_df = pandas.read_csv("data/combined_color_data.csv")
    else:
...
def train_model(data, max_depth=5):
    ...
    X, Y = prepare_data(data, 'color_label')
...
def convert_to_C_code(model, leaf_bits):
    # Do conversion with specified leaf_bits
    c_model = emlearn.convert(model, method='loadable', leaf_bits=leaf_bits)
    c_model.save(file='ml/ColorSenseModelRFC_EML.h', name="color_sense", include_proba=True)
...
model, X_test, Y_test = train_model(color_data, max_depth=6)
convert_to_C_code(model, 5)
